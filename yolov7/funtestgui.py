...
class VideoThread(QThread):
    ...
    def detectThread(self):
        ...
        while True:
            if not self._run_flag:
                break
            self.detectedImage, self.boxes = detect_image_v7.getBoxesFromCvImage(self.image)
            self.switch = not self.switch

    def drawGUI(self, image):
        global cam_size
    ...
    def run(self):
        ...
        thr = threading.Thread(target=self.detectThread)
        thr.start()
        ...
        while True:
            ...
            if width != cam_size[0] or height != cam_size[1]:
                cam.set(cv2.CAP_PROP_FRAME_WIDTH, cam_size[0])
                cam.set(cv2.CAP_PROP_FRAME_HEIGHT, cam_size[1])
            ...
            if(last != self.switch):
                print("IMAGE CHANGED")
                self.change_pixmap_signal.emit(self.drawGUI(self.detectedImage))
